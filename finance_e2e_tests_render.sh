...
#
# Examples:
#   ./finance_e2e_tests_render.sh                         # Tests the deployed Render API
#   ./finance_e2e_tests_render.sh "https://api.example.com/api" # Custom API URL

# Configuration
DEFAULT_BASE_URL="https://finance-manager-2f0q.onrender.com/api"
...
#=============================================================================
# GIT BASH / ENVIRONMENT CHECKS
#=============================================================================

command -v curl >/dev/null 2>&1 || {
    echo -e "${RED}ERROR:${NC} curl is not available. Install/use Git Bash with curl."
    exit 1
}

command -v awk >/dev/null 2>&1 || {
    echo -e "${RED}ERROR:${NC} awk is not available in this Git Bash environment."
    exit 1
}

# Keep cookie files in the current directory and remove them when the script exits.
cleanup() {
    rm -f user1_session.txt user2_session.txt
}
trap cleanup EXIT

# Normalize BASE_URL so the script works whether /api is supplied or not.
BASE_URL="${BASE_URL%/}"
...
validate_response_value() {
    ...
    case "$comparison_type" in
        "equals")
            ...
            ;;
        "greater_than")
            if awk "BEGIN { exit !($actual_value > $expected_value) }"; then
...
api_test() {
    ...
    local curl_cmd=(curl -sS --connect-timeout 15 --max-time 90 --retry 2 --retry-delay 2 -w "%{http_code}")
...
# Fixed future date: current date is 2026-09-06
